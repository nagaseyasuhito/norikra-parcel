...
if [[ "$1" == "activate" ]]; then
  update-alternatives --install /usr/bin/norikra norikra $PARCELS_ROOT/$NORIKRA_DIRNAME/gem/bin/norikra $PRIORITY
  update-alternatives --install /usr/bin/norikra-client norikra-client $PARCELS_ROOT/$NORIKRA_DIRNAME/gem/bin/norikra-client $PRIORITY
